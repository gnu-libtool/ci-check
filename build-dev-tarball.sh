...
(cd "$package" && patch -p1 < ../patches/Skip-test-option-parser.sh-for-ksh-shell-on-NetBSD.patch \
               && patch -p1 < ../patches/0001-ltmain.in-ensure-old-libraries-are-not-archived.patch \
               && patch -p1 < ../patches/0002-tests-extend-archive-in-archive-with-dependency-forw.patch \
               && patch -p1 < ../patches/0003-ltmain.in-defer-warn-dependency-to-static-library.patch )
